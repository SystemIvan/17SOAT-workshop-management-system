...
CREATE TABLE stock_items (
    ...
    PRIMARY KEY (id),
    CONSTRAINT uk_stock_items_sku UNIQUE (sku),
    CONSTRAINT ck_stock_items_price_value CHECK (price_value >= 0),
    CONSTRAINT ck_stock_items_price_currency CHECK (price_currency = 'BRL'),
    CONSTRAINT ck_stock_items_available_quantity CHECK (available_quantity >= 0),
    CONSTRAINT ck_stock_items_type CHECK (type IN ('PART', 'CONSUMABLE', 'SUPPLY'))
);
...
ALTER TABLE technician_specialties ADD CONSTRAINT fk_technician_specialties_technician
    FOREIGN KEY (technician_id) REFERENCES technicians (id);
ALTER TABLE service_order_approved_estimates ADD CONSTRAINT fk_service_order_approved_estimates_order
    FOREIGN KEY (service_order_id) REFERENCES service_orders (id);
ALTER TABLE service_executions ADD CONSTRAINT fk_service_executions_order
    FOREIGN KEY (service_order_id) REFERENCES service_orders (id);
ALTER TABLE service_execution_stock_requirements ADD CONSTRAINT fk_service_execution_stock_requirements_execution
