...
import br.com.fiap.workshop_management_system.servicelifecycle.estimate.domain.service.EstimateExpirationPolicy;
...
public class GenerateEstimateUseCase {
    ...
    private final EstimateExpirationPolicy expirationPolicy;
    ...
    GenerateEstimateUseCase(
            ...
            ApplicationEventPublisher eventPublisher,
            RepairStockAssessmentApi repairStockAssessmentApi,
            EstimateExpirationPolicy expirationPolicy) {
    ...
    public Result execute(UUID serviceOrderId, UUID diagnosisId) {
        ServiceOrder serviceOrder = serviceOrderRepository
                .findByIdForUpdate(serviceOrderId)
        ...
        List<ServiceExecution> executions = serviceOrder
                .serviceExecutions()
                .stream()
                .filter(execution ->
                        diagnosisId.equals(execution.diagnosisId()))
                .toList();
        ...
        if (repairStockAssessmentApi != null) {
            RepairStockAssessmentResult result =
                    repairStockAssessmentApi.assessAndRecord(
                            new RepairStockAssessmentCommand(
                                    toAssessmentExecutions(executions)
                            )
                    );

            serviceOrder.recordStockAvailability(
                    diagnosisId,
                    toSnapshots(result)
            );
        ...
        List<EstimateLine> lines = executions
                .stream()
        ...
        Instant expiresAt =
                expirationPolicy.calculateExpiresAt(
                        createdAt,
                        lines
                );
    ...
    private void validateDiagnosis(
        ...
        if (serviceOrder.openDiagnosisId() == null
            ...
            throw new IllegalStateException(
                    "Diagnosis is not open for ServiceOrder: "
                            + diagnosisId
            );
        }
    }

    private EstimateLine toEstimateLine(
            ServiceExecution execution) {

        List<EstimateStockItem> stockItems =
                execution.stockRequirements()
                        .stream()
                        .map(this::toEstimateStockItem)
                        .toList();

        List<EstimateStockAvailability> stockAvailability =
                execution.stockAvailability()
                        .stream()
                        .map(snapshot ->
                                new EstimateStockAvailability(
                                        snapshot.stockItemId(),
                                        snapshot.requestedQuantity(),
                                        snapshot.observedAvailableQuantity(),
                                        snapshot.shortageQuantity(),
                                        snapshot.status(),
                                        snapshot.observedAt()
                                ))
                        .toList();
    ...
    private EstimateStockItem toEstimateStockItem(
            StockRequirement requirement) {
    ...
    private List<RepairStockAssessmentExecution>
    toAssessmentExecutions(List<ServiceExecution> executions) {

        return executions.stream()
                .map(execution ->
                        new RepairStockAssessmentExecution(
                                execution.id(),
                                execution.stockRequirements()
                                        .stream()
                ...
                .filter(execution ->
                        !execution.lines().isEmpty())
                .toList();
    }

    private Map<UUID, List<StockAvailabilitySnapshot>>
    toSnapshots(RepairStockAssessmentResult result) {

        return result.executions()
                .stream()
                .collect(Collectors.toMap(
                        execution ->
                                execution.serviceExecutionId(),

                        execution ->
                                execution.lines()
                                        .stream()
                                        .map(line ->
                                                new StockAvailabilitySnapshot(
                                                        line.stockItemId(),
                                                        line.requestedQuantity(),
                                                        line.observedAvailableQuantity(),
                                                        line.shortageQuantity(),
                                                        StockAvailabilityStatus.valueOf(
                                                                line.status().name()
                                                        ),
                                                        line.observedAt()
                                                ))
