...
/**
 * RF12 - HTTP coverage for {@code POST /api/service-orders/{id}/executions/{executionId}/stock-requirements}.
 * Completion, like in {@link ServiceOrderControllerAssignTechnicianTest}, is built by calling the aggregate
 * directly through {@link ServiceOrderRepository} since authorization is an Epic 2 domain-only method today.
 */
@SpringBootTest
class ServiceOrderControllerAttachStockRequirementTest {
    ...
    @Test
    void attachesStockRequirementToAPendingExecutionAndReturns200() throws Exception {
        ...
        mockMvc.perform(post("/api/service-orders/{id}/executions/{executionId}/stock-requirements",
                        ...
                        .content(stockRequirementBody()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.executions[0].stockRequirements.length()").value(1))
                .andExpect(jsonPath("$.executions[0].stockRequirements[0].reserved").value(false));
    }
    ...
    @Test
    void returnsValidationErrorWhenQuantityIsZero() throws Exception {
        String serviceOrderId = createServiceOrder();
        String executionId = diagnoseWithOneExecution(serviceOrderId);
    ...
    @Test
    void returnsValidationErrorWhenFieldsAreMissing() throws Exception {
        String serviceOrderId = createServiceOrder();
        String executionId = diagnoseWithOneExecution(serviceOrderId);
    ...
    private static String stockRequirementBody() {
        return """
                {
                  "stockItemId": "%s",
                  "type": "PART",
