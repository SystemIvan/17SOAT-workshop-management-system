...
- `docs/features/context-alignment-and-project-standards/`
...
- [ ] Peças, consumíveis e insumos podem ser cadastrados como Stock Items de tipo explícito.
- [ ] Cada item possui ID, SKU único, nome, tipo, preço `BRL`, quantidade disponível e estado ativo.
- [ ] É possível consultar por ID um item ativo ou inativo.
- [ ] Sem filtros adicionais, a visão do Technician lista todos os Stock Items ativos.
- [ ] É possível combinar texto, múltiplos tipos, disponibilidade e estado ativo.
- [ ] A busca encontra correspondências de nome ou SKU sem diferenciar maiúsculas e minúsculas.
- [ ] A listagem oferece dados suficientes para o Technician escolher item e quantidade durante o diagnóstico.
- [ ] Atualização altera somente nome e preço.
- [ ] Desativação é lógica e não permite atualização posterior.
- [ ] Nenhuma consulta modifica ou reserva quantidade.
- [ ] Nenhum endpoint, modelo ou tabela representa um Stock físico com ID.
- [ ] `/api/parts` é removido e o contrato `/api/stock-items` é documentado em OpenAPI e Postman.
- [ ] Nenhuma operação de inventário, reserva ou Procurement é implementada nesta feature.
